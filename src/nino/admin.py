...
class NNAAdmin(admin.ModelAdmin):
    ...
    list_display = ('nombre_nino','apellido_nino','sexo','motivo_ingreso','fecha_nacimiento','cui','Fotografia','orden_ingreso','Certificado',)
    list_filter = ('fecha_evaluacion','motivo_ingreso',)
    search_fields = ('nombre_nino','apellido_nino','cui',)
    readonly_fields = ("Fotografia","Certificado",)
    ...
    def Certificado(self, obj):
        return mark_safe(
            '<img src="{url}" style="max-width: 100px"/>'.format(url=obj.certificado_nacimiento.url)
        )
